...
// todo: document that new options are for animated WEBPs only
  ...
  if (is.object(options) && is.defined(options.pageDelay)) {
    if (
      Array.isArray(options.pageDelay) &&
      options.pageDelay.every(is.integer) &&
      options.pageDelay.every(v => v >= 0 && v <= 65535)
    ) {
      this.options.pageDelay = options.pageDelay;
    } else {
      throw new Error('Invalid pageDelay. ' + options.pageDelay +' must be an array 0-65535 integers.');
    }
  }
  if (is.object(options) && is.defined(options.pageLoop)) {
  ...
  if (is.object(options) && is.defined(options.reductionEffort)) {
    if (is.integer(options.reductionEffort) && is.inRange(options.reductionEffort, 0, 6)) {
      ...
      throw new Error('Invalid webp reduction effort (integer, 0-6) ' + options.reductionEffort);
    }
  }
  ...
  if (is.object(options) && is.defined(options.minSize)) {
 ...
 * @param {Array<Number>} [options.pageDelay] - delay between each page, in miliseconds
